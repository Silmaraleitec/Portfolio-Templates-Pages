...
const Page = () => {
	...
	return (
		<PageContainer>
			...
			<Link className="crumb" to={`/ads?state=${adInfo.stateName}`}>{adInfo.stateName}</Link> /
			<Link className="crumb" to={`/ads?state=${adInfo.stateName} &cat=${adInfo.category.slug}`}>{adInfo.category.name}</Link> / {adInfo.title}
			</BreadCrumb>}
			<PageArea>
				...
				<div className="rightSide">
					...
					<div className="price"> Preço: <span>R$ {adInfo.price}</span></div>}
					 ...
					 {adInfo.userInfo &&
					 <>
					 <a href={`mailto${adInfo.userInfo.email}`} target="_blank" 
